...
class SimpleTeleopNode(Node):
    ...
    def __init__(self):
        ...
        # Load poses from profiles.csv
        profiles_path = os.path.join(os.getcwd(), "profile_points.csv")
        if not os.path.exists(profiles_path):
            self.get_logger().error(f"profile_points.csv not found at {profiles_path}")
            return
        df_profiles = pd.read_csv(profiles_path)

        # Concatenate DataFrames for easier indexing
        df_points.reset_index(drop=True, inplace=True)
        df_profiles.reset_index(drop=True, inplace=True)

        # Extract first 4 and last 4 from each
        df_grasp = pd.concat([df_points.iloc[:1], df_profiles.iloc[-1:]], ignore_index=True)
        df_insert = pd.concat([df_points.iloc[-1:], df_profiles.iloc[:1]], ignore_index=True)

        # Helper to convert DataFrame rows to Pose list
        def df_to_poses(df):
            poses = []
        ...
        first_four = list(reversed(df_to_poses(df_grasp)))
        last_four = list(reversed(df_to_poses(df_insert)))  

        self.sequence = []
        for i in range(max(len(first_four),len(last_four))):
            self.sequence.append((first_four[i], True))   # grasp after these
            if i < len(last_four):
                self.sequence.append((last_four[i], False))   # no grasp after these
    ...
    def run(self):
        ...
        for i, (pose, do_grasp) in enumerate(self.sequence):
            ...
            pre_pose.orientation = pose.orientation
              ...
              # --- Rotate orientation by 180 deg around Z and move a few mm in -Z before moving down ---
            pose_rotated = Pose()
            pose_rotated.position = pre_pose.position
            quat = [
                pose.orientation.x,
                pose.orientation.y,
                pose.orientation.z,
            ...
            r = R.from_quat(quat)
            r_rot = R.from_euler('z', 180, degrees=True)
            r_new = r * r_rot  # Apply rotation in Z
            quat_new = r_new.as_quat()  # [x, y, z, w]
            pose_rotated.orientation.x = quat_new[0]
            pose_rotated.orientation.y = quat_new[1]
            pose_rotated.orientation.z = quat_new[2]
            pose_rotated.orientation.w = quat_new[3]
            ...
            if do_grasp:
                ...
                # TODO: profile insert works only with HIGH stiffness, while screws only with LOW stiffness
                self.cartesian_pub.publish(pre_pose)
                self.wait(3.5)
                # LOG
                error = [self.ee_pose.position.x - pose.position.x,
