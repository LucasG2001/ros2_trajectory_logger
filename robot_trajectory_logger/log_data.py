...
class RobotTrajectoryLogger(Node):
    ...
    def robot_state_callback(self, msg: FrankaRobotState):
        ...
        try:
            self.joint_velocities = list(msg.measured_joint_motor_state.velocity)
    ...
    def log_data(self):
        ...
        # Ensure required data is available before logging
        if self.f_ext is None or self.reference_pose is None or self.ee_euler_angles is None:
            self.get_logger().warn("Required data missing. Skipping log entry.")
            return
        ...
        # Default to empty lists if jacobian data is None
        jacobianEE_data = list(self.jacobianEE) if self.jacobianEE is not None else []
        dtjacobianEE_data = list(self.dtjacobianEE) if self.dtjacobianEE is not None else []
        joint_velocities_data = list(self.joint_velocities) if self.joint_velocities is not None else []
